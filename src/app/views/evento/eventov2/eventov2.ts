...
import { EstimacionDetalle, EventoV2DetailsFilesComponent, EventoV2Requisito } from './components/evento-v2-details-files/evento-v2-details-files';
import { EventoV2ActivityComponent } from './components/evento-v2-activity/evento-v2-activity';
import { EventoCompleto, EventoDocumentacion, EventoVistaResponse, Evento_requisito_completo, VidaEvento, actividadesMock, adjuntosMock, documentacionMock, eventoMock, requisitosMock } from '@core/interfaces/evento';
...
@Component({
  ...
  templateUrl: './eventov2.html',
  styleUrl: './eventov2.scss',
})
export class Eventov2 {
  ...
  loadingMap: Record<'evento' | 'actividad' | 'adjuntos' | 'requisitos', boolean> = {
    evento: false,
    actividad: false,
    adjuntos: false,
    requisitos: false,
  ...
  loadEventoBase(force = false): void {
    ...
    this.eventoService.getEventoVista(this.eventoId).pipe(
      finalize(() => this.finalizeBlockLoading('evento'))
    ).subscribe({
      next: (res) => {
  ...
  loadActividad(force = false): void {
    ...
    this.eventoService.getEventoVista(this.eventoId, { incluirActividad: true }).pipe(
      finalize(() => this.finalizeBlockLoading('actividad'))
    ).subscribe({
      next: (res) => {
  ...
  loadAdjuntos(force = false): void {
    ...
    this.eventoService.getEventoVista(this.eventoId, { incluirAdjuntos: true, adjuntosActivo: 'all' }).pipe(
      finalize(() => this.finalizeBlockLoading('adjuntos'))
    ).subscribe({
      next: (res) => {
  ...
  loadRequisitos(force = false): void {
    ...
    this.eventoService.getEventoVista(this.eventoId, { incluirRequisitos: true }).pipe(
      finalize(() => this.finalizeBlockLoading('requisitos'))
    ).subscribe({
      next: (res) => {
  ...
  private finalizeBlockLoading(block: 'evento' | 'actividad' | 'adjuntos' | 'requisitos'): void {
    this.zone.run(() => {
  ...
  private mapRequisitos(requisitos: Evento_requisito_completo[]): EventoV2Requisito[] {
    return requisitos.map((req) => ({
      cumplido: req.cumplido,
      requisito: req.requisito?.descripcion ?? req.requisito?.codigo ?? 'Sin descripción',
  ...
  private resolveRequisitoValor(req: Evento_requisito_completo): string {
    ...
    if (cumplimiento.valor !== undefined && cumplimiento.valor !== null) return String(cumplimiento.valor);
    if (cumplimiento.valorTexto !== undefined && cumplimiento.valorTexto !== null) return String(cumplimiento.valorTexto);
    if (cumplimiento.valorNumero !== undefined && cumplimiento.valorNumero !== null) return String(cumplimiento.valorNumero);
    if (cumplimiento.valorFecha) return String(cumplimiento.valorFecha);
    if (cumplimiento.valorBooleano !== undefined && cumplimiento.valorBooleano !== null) {
  ...
  private loadActividadLegacy(force = false): void {
    ...
    this.eventoService.getActividad(this.eventoId).subscribe({
      ...
      error: () => {
        this.loadingActividad = false;
        this.errorActividad = 'No se pudo cargar la actividad (fallback legacy).';
      },
    });
  }
  ...
  private loadAdjuntosLegacy(force = false): void {
    ...
    this.eventoService.getAdjuntos(this.eventoId).subscribe({
      ...
      error: () => {
        this.loadingAdjuntos = false;
        this.errorAdjuntos = 'No se pudieron cargar los adjuntos (fallback legacy).';
      },
    });
  }
  ...
  private loadRequisitosLegacy(force = false): void {
    ...
    this.eventoService.getRequisitos(this.eventoId).subscribe({
      ...
      error: () => {
        this.loadingRequisitos = false;
        this.errorRequisitos = 'No se pudieron cargar los requisitos (fallback legacy).';
      },
    });
  }
